...
void calcola_aux(Stack *&, Stack *&,const int, int i=0);
int getNth(Stack *&, int);
...
void calcola(Stack *&s1, Stack *&s2){
    ...
    const int DIM=lenght(s1);
    return calcola_aux(s1, s2, DIM);
}
void calcola_aux(Stack *&s1, Stack *&s2, const int DIM, int i){
    if(i==DIM) return;
    int n=pop(s1);
    int n2=getNth(s2, DIM-i-1);
    calcola_aux(s1, s2, DIM, i+1);
    push(s1, (n*n2));
    return;
}


int getNth(Stack *&s, int i){
    int n=pop(s);
    if(i==0){ push(s,n); return n;}
    int res=getNth(s,i-1);
    push(s, n);
    return res;
}
