...
Queue * calcola(Queue *&);
void calcola_aux(Queue *&q,Queue *& res, int *);
void initArr(int *, const int dim, int i=0);
void getOcc(Queue *&, int *);
...
Queue * calcola(Queue *&q){
    Queue *res;
    res=initQueue();
    if(isEmpty(q)) return res;
    int v[10]; //al massimo valori tra 0-9
    initArr(v,10);
    getOcc(q,v);
    calcola_aux(q, res,v);
    reverse(res);
...
void calcola_aux(Queue *&q,Queue *& res, int *v){
    if(isEmpty(q)) return;
    int n=dequeue(q);
    enqueue(res,v[n]);
    enqueue(res,n);
...
void initArr(int *v, const int dim, int i){ //sono un coglione, potevo usare i cicli ma mi sono dimenticato
    if(i==dim)  return;
    v[i]=0;
    initArr(v,dim, i+1);
}
