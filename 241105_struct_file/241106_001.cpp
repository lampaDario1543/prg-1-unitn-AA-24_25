/*
Aggiungere al programma persona/persona2/persona3 una funzionalita'
...
bool inserisci_persona_gruppo (const persona & p, gruppo & g,bool isOnFile);
...
int main ()
{
  ...
  do {
    ...
    case 'i': leggi_persona(p);
      if (! inserisci_persona_gruppo(p,g,false))
	cout << "Gruppo pieno\n";
      break;
...
void inizializza_gruppo (gruppo &g)
{
  g.npers=0;
  fstream in;
  in.open("rubrica.txt", ios::in);
  if (in.fail()){
    //creo il file rubrica.txt
    in.open("rubrica.txt", ios::out);
    in.close();
    g.npers=0;
    return;
  }
  ...
  //leggo il file
  while(!in.eof()){
    in >> p->nome >> p->cognome >> p->indirizzo.via >> p->indirizzo.civico >> p->indirizzo.comune >> p->indirizzo.cap >> p->indirizzo.prov; 
    inserisci_persona_gruppo(*p,g,true);
    //stampa_persona(*p);
    ++ctr;
  }
  in.close();
...
bool inserisci_persona_gruppo (const persona & p, gruppo & g, bool isOnFile)
{
  ...
  if (g.npers==NmaxPers)
    ...
    if (out.fail()){
      cerr << "Errore apertura file rubrica.txt" << endl;
      exit(0);
    }
    persona * tmp;
    tmp = new(persona);
    *tmp = p;
    if(!isOnFile)
      out <<endl<< tmp->nome << " "<< tmp->cognome<< " " << tmp->indirizzo.via << " "<< tmp->indirizzo.civico<< " " << tmp->indirizzo.comune << " "<< tmp->indirizzo.cap << " "<< tmp->indirizzo.prov;
    g.nomi[g.npers] = tmp;
