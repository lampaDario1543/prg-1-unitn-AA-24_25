...
bool check_assignment(const Dimacs &d, const Assignment &a);
...
bool check_clause(const List * cl, const Assignment & a) {
    if (cl == nullptr) return false;
    bool res = a.values[abs(cl->lit)-1];
    if (cl->lit < 0) res = !res;
    return (res || check_clause(cl->next, a));
}

bool check_clauses(const cnf_t * cls, const Assignment & a) {
    if (cls == nullptr) return true;
    return (check_clause(cls->clause, a) && check_clauses(cls->next, a));
}

bool check_assignment(const Dimacs & d, const Assignment & a) {
    return !check_clauses(d.clauses, a);
}
