...
double guadagno(Abbonamento *abb, int N)
{
    double guadagno = 0;
    for (int i = 0; i < N; i++)
    {
        guadagno += abb[i].costo;
    }
    return 0.9 * guadagno;
}
...
int main(int argc, char **argv)
{
    ...
    double guadagnototale = guadagno(abb, N);

    cout << "Guadagno totale: " << guadagnototale << endl;

    double guadagniparziali[4];

    char tipo = elabora(abb, N, guadagniparziali);
