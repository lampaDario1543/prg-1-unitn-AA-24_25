...
void calcola_aux(Stack *&s1, Stack *&s2, int, const int);
int getValue(Stack *&, int, const int);
...
void calcola(Stack *&s1, Stack *&s2)
{
    if (isEmpty(s1))
        return;
    return calcola_aux(s1, s2, 0, lenght(s1));
}
void calcola_aux(Stack *&s1, Stack *&s2, int i, const int DIM)
{
    if (i == DIM)
        return;
    const int val_s1 = pop(s1);
    const int val_s2 = getValue(s2, 1, DIM - i);
    calcola_aux(s1, s2, i + 1, DIM);
    push(s1, val_s1 * val_s2);
}
int getValue(Stack *&s, int i, const int DIM)
{
    if (i == DIM)
        return top(s);
    int n = pop(s);
    int res = getValue(s, i + 1, DIM);
    push(s, n);
    return res;
}
