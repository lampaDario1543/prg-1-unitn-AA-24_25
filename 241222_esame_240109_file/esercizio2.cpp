...
void conta(char *A, const int D,int i, const int j, int *&res);
void conta_aux(char *A, const int D,int i, const int j, int *&res, int occ=0);
...
void conta(char *A, const int D,int i, const int j, int *&res){
  ...
  return conta_aux(A, D, i, j, res);
}

void conta_aux(char *A, const int D,int i, const int j, int *&res,int occ){
  if(i==j){
    res= new int [occ+1];
    res[0]=occ;
    return;
  ...
  if(A[i]==A[j]){
    conta_aux(A,D,i+1,j, res,occ+1);
    res[occ+1]=i;
    return;
  }
  conta_aux(A,D,i+1,j, res,occ);
