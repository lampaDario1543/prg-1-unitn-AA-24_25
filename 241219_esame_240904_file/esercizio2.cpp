...
void calcola_aux(Stack *&s1, Stack *&s2, List *&, bool);
...
void calcola_aux(Stack *&s1, Stack *&s2, List *&res, bool flag)
{
    if (isEmpty(s1))
        return;
    int n1 = pop(s1);
    int n2 = pop(s2);
    if (flag)
        insert_inorder(res, n1);
    else
        insert_inorder(res, n2);
    calcola_aux(s1, s2, res, !flag);
    push(s1,n1);
    push(s2,n2);
}
