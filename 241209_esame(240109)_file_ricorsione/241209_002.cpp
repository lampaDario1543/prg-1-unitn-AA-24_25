...
void conta(char *, const int, const int, const int, int *&);
void conta(char *,const int, int, const int, int *&, int ctr);
...
void conta(char *A, const int D, int i, const int J, int *&res){
  return conta(A,D,i,J,res,0);
}
void conta(char *A, const int D, int i, const int J, int *&res, int ctr){
  if(i==J){
    res=new int[ctr+1];
    res[0]=ctr;
    return;
  ...
  if(A[i]==A[J]){
    conta(A,D,i+1,J,res,ctr+1);
    res[ctr+1]=i;
    return;
  }
  conta(A,D,i+1,J,res,ctr);
  return;
}
/* Inserire qui sotto la definizione della funzione conta */
